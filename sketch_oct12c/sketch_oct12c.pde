...
void normalize() {
  ...
  // value in deadzone => norm = 0
  if ((joy_x_raw >= deadzone_x_min) && (joy_x_raw <= deadzone_x_max)) {
    joy_x_norm = 0;
  }
  //
  if (joy_x_raw < deadzone_x_min) {
    joy_x_norm = map(joy_x_raw, joy_y_min, deadzone_x_min, -255, 0);
  }
  //
  if (joy_x_raw > deadzone_x_max) {
    joy_x_norm = map(joy_x_raw, deadzone_x_max, joy_x_max, 0,255);
  }
